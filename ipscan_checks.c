...
int check_icmpv6_echoresponse(char * hostname, uint64_t starttime, uint64_t session)
{
	...
	// TODO delete me? struct cmsghdr *cmsghdr_ptr;
	...
	// TODO delete me http://cvsweb.netbsd.org/bsdweb.cgi/~checkout~/src/sbin/ping6/ping6.c?rev=1.78&content-type=text/plain&only_with_tag=MAIN
	...
	while ( ((timenow - timestart) <= 1+TIMEOUTSECS) && foundit == 0)
	{
		...
		else
		{
			...
			// sent: snprintf(&txpackdata[ICMP6DATAOFFSET],(ICMPV6_PACKET_SIZE-ICMP6DATAOFFSET),"%"PRId64" %"PRId64" 010892 210695", starttime, session);
