...
#include <inttypes.h>
	...
	#define UDPRUNTIMEPARLL (((int)((int)numudpports / (int)MAXUDPPORTSPERCHILD) * UDPTIMEOUTSECS) + ((int)(numudpports % MAXUDPPORTSPERCHILD) * UDPTIMEOUTSECS) + UDPSTATICTIME)
	#define UDPRUNTIMESERL (numudpports * UDPTIMEOUTSECS + UDPSTATICTIME)
	#define UDPRUNTIME ((MAXUDPCHILDREN == 1) ? UDPRUNTIMESERL :  UDPRUNTIMEPARLL )
	#else
	#define UDPRUNTIME 0
	#endif
	//
	#define TCPRUNTIMEPARLL (((int)((int)numports / (int)MAXPORTSPERCHILD) * TIMEOUTSECS) + ((numports % MAXPORTSPERCHILD) * TIMEOUTSECS) + TCPSTATICTIME)
	#define TCPRUNTIMESERL (numports * TIMEOUTSECS + TCPSTATICTIME)
	#define TCPRUNTIME ( (MAXCHILDREN == 1) ? TCPRUNTIMESERL : TCPRUNTIMEPARLL )
